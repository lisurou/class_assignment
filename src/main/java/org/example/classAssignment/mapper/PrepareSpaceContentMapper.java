...
public interface PrepareSpaceContentMapper {
    @Select("<script>" +
            "select folder_id as folderId, owner_id as courseId, category, parent_id as parentId, name, created_by as createdBy, created_at as createdAt " +
            "from resource_folder " +
            "where owner_type='PREPARE_SPACE' and owner_id=cast(#{spaceId} as char) and module=#{module} " +
    ...
    @Select("select folder_id as folderId, owner_id as courseId, category, parent_id as parentId, name, created_by as createdBy, created_at as createdAt " +
            "from resource_folder " +
            "where owner_type='PREPARE_SPACE' and owner_id=cast(#{spaceId} as char) and module=#{module} and folder_id=#{folderId} " +
            "limit 1")
    MaterialFolder findFolderById(@Param("spaceId") Long spaceId, @Param("module") String module, @Param("folderId") Long folderId);

    @Insert("insert into resource_folder(owner_type, owner_id, module, category, parent_id, name, created_by, created_at) " +
            "values('PREPARE_SPACE', cast(#{spaceId} as char), #{module}, #{folder.category}, #{folder.parentId}, #{folder.name}, #{createdBy}, now())")
    ...
    @Update("update resource_folder set parent_id=#{parentId} where owner_type='PREPARE_SPACE' and owner_id=cast(#{spaceId} as char) and module=#{module} and folder_id=#{folderId}")
    Integer updateFolderParent(@Param("spaceId") Long spaceId,
                               @Param("module") String module,
                               @Param("folderId") Long folderId,
                               @Param("parentId") Long parentId);

    @Delete("delete from resource_folder where owner_type='PREPARE_SPACE' and owner_id=cast(#{spaceId} as char) and module=#{module} and folder_id=#{folderId}")
    ...
    @Select("<script>" +
            "select resource_id as coursewareId, folder_id as folderId, title, item_type as itemType, visibility, url, " +
            ...
            "from prepare_space_resource_item " +
            "where prepare_space_id=#{spaceId} and module='COURSEWARE' and resource_kind='COURSEWARE' " +
    ...
    @Select("select resource_id as coursewareId, folder_id as folderId, title, item_type as itemType, visibility, url, " +
            "original_name as originalName, stored_name as storedName, relative_path as relativePath, size, content_type as contentType, " +
            "created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='COURSEWARE' and resource_kind='COURSEWARE' and resource_id=#{coursewareId} limit 1")
    ...
    @Select("select resource_id as coursewareId, folder_id as folderId, title, item_type as itemType, visibility, url, " +
            ...
            "created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item " +
            "where prepare_space_id=#{spaceId} and module='COURSEWARE' and resource_kind='COURSEWARE' and folder_id=#{folderId} " +
            "order by created_at desc, resource_id desc")
    List<Courseware> findCoursewaresByFolder(@Param("spaceId") Long spaceId, @Param("folderId") Long folderId);

    @Insert("insert into prepare_space_resource_item(prepare_space_id, module, category, folder_id, resource_kind, title, item_type, visibility, url, original_name, stored_name, relative_path, size, content_type, created_by, created_at) " +
            "values(#{spaceId}, 'COURSEWARE', null, #{courseware.folderId}, 'COURSEWARE', #{courseware.title}, #{courseware.itemType}, #{courseware.visibility}, #{courseware.url}, " +
    ...
    @Options(useGeneratedKeys = true, keyProperty = "courseware.coursewareId", keyColumn = "resource_id")
    Boolean insertCourseware(@Param("spaceId") Long spaceId,
                             @Param("createdBy") String createdBy,
                             @Param("courseware") Courseware courseware);

    @Delete("delete from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='COURSEWARE' and resource_kind='COURSEWARE' and resource_id=#{coursewareId}")
    Integer deleteCourseware(@Param("spaceId") Long spaceId, @Param("coursewareId") Long coursewareId);

    @Select("<script>" +
            "select resource_id as attachmentId, category, folder_id as folderId, original_name as originalName, stored_name as storedName, " +
            "relative_path as relativePath, size, content_type as contentType, created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item " +
            "where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='FILE' " +
            "<if test='category != null'> and category=#{category}</if> " +
            "<if test='folderId != null'> and folder_id=#{folderId}</if> " +
    ...
    @Select("select resource_id as attachmentId, category, folder_id as folderId, original_name as originalName, stored_name as storedName, " +
            "relative_path as relativePath, size, content_type as contentType, created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item " +
            "where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='FILE' and resource_id=#{attachmentId} limit 1")
    MaterialAttachment findMaterialAttachmentById(@Param("spaceId") Long spaceId, @Param("attachmentId") Long attachmentId);

    @Select("select resource_id as attachmentId, category, folder_id as folderId, original_name as originalName, stored_name as storedName, " +
            "relative_path as relativePath, size, content_type as contentType, created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item " +
            "where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='FILE' and folder_id=#{folderId} order by created_at desc, resource_id desc")
    List<MaterialAttachment> findMaterialAttachmentsByFolder(@Param("spaceId") Long spaceId, @Param("folderId") Long folderId);

    @Insert("insert into prepare_space_resource_item(prepare_space_id, module, category, folder_id, resource_kind, title, original_name, stored_name, relative_path, size, content_type, created_by, created_at) " +
            "values(#{spaceId}, 'MATERIAL', #{attachment.category}, #{attachment.folderId}, 'FILE', #{attachment.originalName}, #{attachment.originalName}, #{attachment.storedName}, #{attachment.relativePath}, #{attachment.size}, #{attachment.contentType}, #{createdBy}, now())")
    @Options(useGeneratedKeys = true, keyProperty = "attachment.attachmentId", keyColumn = "resource_id")
    Boolean insertMaterialAttachment(@Param("spaceId") Long spaceId,
                                    @Param("createdBy") String createdBy,
                                    @Param("attachment") MaterialAttachment attachment);

    @Delete("delete from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='FILE' and resource_id=#{attachmentId}")
    Integer deleteMaterialAttachment(@Param("spaceId") Long spaceId, @Param("attachmentId") Long attachmentId);

    @Select("<script>" +
            "select resource_id as linkId, category, folder_id as folderId, title, url, created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item " +
            "where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='LINK' " +
            "<if test='category != null'> and category=#{category}</if> " +
            "<if test='folderId != null'> and folder_id=#{folderId}</if> " +
            "<if test='keyword != null and keyword != \"\"'> and title like concat('%',#{keyword},'%')</if> " +
            "order by created_at desc, resource_id desc" +
    ...
    @Select("select resource_id as linkId, category, folder_id as folderId, title, url, created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='LINK' and resource_id=#{linkId} limit 1")
    MaterialLink findMaterialLinkById(@Param("spaceId") Long spaceId, @Param("linkId") Long linkId);

    @Select("select resource_id as linkId, category, folder_id as folderId, title, url, created_by as createdBy, created_at as createdAt " +
            "from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='LINK' and folder_id=#{folderId} order by created_at desc, resource_id desc")
    List<MaterialLink> findMaterialLinksByFolder(@Param("spaceId") Long spaceId, @Param("folderId") Long folderId);

    @Insert("insert into prepare_space_resource_item(prepare_space_id, module, category, folder_id, resource_kind, title, url, created_by, created_at) " +
            "values(#{spaceId}, 'MATERIAL', #{link.category}, #{link.folderId}, 'LINK', #{link.title}, #{link.url}, #{createdBy}, now())")
    @Options(useGeneratedKeys = true, keyProperty = "link.linkId", keyColumn = "resource_id")
    Boolean insertMaterialLink(@Param("spaceId") Long spaceId,
                              @Param("createdBy") String createdBy,
                              @Param("link") MaterialLink link);

    @Delete("delete from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='MATERIAL' and resource_kind='LINK' and resource_id=#{linkId}")
    ...
    @Select("select topic_id as topicId, folder_id as folderId, author_id as authorId, author_name as authorName, title, content, is_anonymous as isAnonymous, " +
            ...
            "from discussion_topic where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) order by is_pinned desc, update_time desc")
    List<Topic> findTopics(@Param("spaceId") Long spaceId);

    @Select("select topic_id as topicId, folder_id as folderId, author_id as authorId, author_name as authorName, title, content, is_anonymous as isAnonymous, " +
            "is_pinned as isPinned, is_locked as isLocked, reply_count as replyCount, create_time as createTime, update_time as updateTime " +
            "from discussion_topic where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and folder_id=#{folderId} " +
            "order by is_pinned desc, update_time desc")
    List<Topic> findTopicsByFolder(@Param("spaceId") Long spaceId, @Param("folderId") Long folderId);

    @Select("select topic_id as topicId, folder_id as folderId, author_id as authorId, author_name as authorName, title, content, is_anonymous as isAnonymous, " +
            "is_pinned as isPinned, is_locked as isLocked, reply_count as replyCount, create_time as createTime, update_time as updateTime " +
            "from discussion_topic where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId} limit 1")
    ...
    @Insert("insert into discussion_topic(topic_id, scene_type, scene_id, folder_id, author_id, author_name, title, content, is_anonymous, is_pinned, is_locked, reply_count, create_time, update_time) " +
            "values(#{topicId}, 'PREPARE_SPACE', cast(#{spaceId} as char), #{folderId}, #{authorId}, #{authorName}, #{title}, #{content}, #{isAnonymous}, #{isPinned}, #{isLocked}, #{replyCount}, now(), now())")
    ...
    @Update("update discussion_topic set title=#{title}, content=#{content}, update_time=now() where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId}")
    ...
    @Delete("delete from discussion_topic where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId}")
    Integer deleteTopic(@Param("spaceId") Long spaceId, @Param("topicId") String topicId);

    @Update("update discussion_topic set is_pinned=#{isPinned}, update_time=now() where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId}")
    Integer updateTopicPin(@Param("spaceId") Long spaceId, @Param("topicId") String topicId, @Param("isPinned") Boolean isPinned);

    @Update("update discussion_topic set is_locked=#{isLocked}, update_time=now() where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId}")
    ...
    @Select("select reply_id as replyId, topic_id as topicId, author_id as authorId, author_name as authorName, content, is_anonymous as isAnonymous, create_time as createTime " +
            "from discussion_reply where topic_id=#{topicId} order by create_time asc, reply_id asc")
    List<Reply> findReplies(@Param("spaceId") Long spaceId, @Param("topicId") String topicId);

    @Select("select reply_id as replyId, topic_id as topicId, author_id as authorId, author_name as authorName, content, is_anonymous as isAnonymous, create_time as createTime " +
            "from discussion_reply where reply_id=#{replyId} limit 1")
    ...
    @Insert("insert into discussion_reply(reply_id, topic_id, author_id, author_name, content, is_anonymous, create_time) " +
            "values(#{replyId}, #{topicId}, #{authorId}, #{authorName}, #{content}, #{isAnonymous}, now())")
    ...
    @Update("update discussion_topic set reply_count=reply_count+1, update_time=now() where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId}")
    Integer incrementReplyCount(@Param("spaceId") Long spaceId, @Param("topicId") String topicId);

    @Update("update discussion_topic set reply_count=case when reply_count>0 then reply_count-1 else 0 end, update_time=now() where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and topic_id=#{topicId}")
    ...
    @Delete("delete from prepare_space_resource_item where prepare_space_id=#{spaceId} and module='COURSEWARE' and resource_kind='COURSEWARE' and folder_id=#{folderId}")
    Integer deleteCoursewareByFolder(@Param("spaceId") Long spaceId, @Param("folderId") Long folderId);

    @Delete("delete from discussion_topic where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char) and folder_id=#{folderId}")
    Integer deleteTopicsByFolder(@Param("spaceId") Long spaceId, @Param("folderId") Long folderId);

    @Delete("delete from prepare_space_assignment_submission where prepare_space_id=#{spaceId}")
    Integer deleteAllAssignmentSubmissionsBySpaceId(@Param("spaceId") Long spaceId);

    @Delete("delete from prepare_space_assignment where prepare_space_id=#{spaceId}")
    Integer deleteAllAssignmentsBySpaceId(@Param("spaceId") Long spaceId);

    @Delete("delete from prepare_space_resource_item where prepare_space_id=#{spaceId}")
    Integer deleteAllResourcesBySpaceId(@Param("spaceId") Long spaceId);

    @Delete("delete from discussion_topic where scene_type='PREPARE_SPACE' and scene_id=cast(#{spaceId} as char)")
    Integer deleteAllTopicsBySpaceId(@Param("spaceId") Long spaceId);

    @Delete("delete from prepare_space_import_log where prepare_space_id=#{spaceId}")
    Integer deleteAllImportLogsBySpaceId(@Param("spaceId") Long spaceId);

    @Delete("delete from resource_folder where owner_type='PREPARE_SPACE' and owner_id=cast(#{spaceId} as char)")
    Integer deleteAllFoldersBySpaceId(@Param("spaceId") Long spaceId);
}
